...
class DiffusionSamplerLite(LightningLite):
    def run(self, cfg) -> Any:
        ...
        diffusion_model = DenoisingDiffusionProbabilisticModel(eps_model=diffusion_model) # 将加载权重后的UNet包装成完整的DDPM模型
        ...
        size = latent_encoder(torch.ones([1, *size])).shape[-3:] # 创建形状为(1, 3, 128, 128)的全1张量，通过编码器得到潜在表示，取最后3个维度
        # del latent_encoder
        ...
        sorted_keys_values = sorted(contexts.items(),key=lambda x:int(x[0].split('/')[1]))
        sorted_keys = [key for key,value in sorted_keys_values]
        sorted_values = [value for key,value in sorted_keys_values]
        contexts = np.stack(sorted_values)
        contexts = contexts[:10000]
        contexts_norm = np.linalg.norm(contexts,axis=1)
        contexts = contexts / contexts_norm[:, np.newaxis] # 使得每个向量长度为1
