...
from omegaconf import OmegaConf, DictConfig # 与Hydra配套使用，处理YAML配置文件，支持类型检查
...
class DiffusionTrainerLite(LightningLite):
    ...
    def save_checkpoint(ema_model, diffusion_model, optimizer, global_step, epoch, steps_of_checkpoints=None, lr_scheduler=None):
        ...
        torch.save(diffusion_model.state_dict(), ensure_path_join(os.getcwd(), 'checkpoints', f'model_{global_step}.ckpt'))
    ...
    # 这段代码是测试用的，用了各种sample的setting，一共五种
    # 创建包含多种采样情况的图像网格
    @rank_zero_only
    def create_and_save_sample_grid(self, model, size, x=None, context=None, latent_decoder=None, save_path=''):
        # constants
        N_PER_ROW = 2 # 这个是N_PER_BLOCK的因数 每行显示2张图片
        PAD_VALUE = 1.0 # 填充区域颜色为白色
        PADDING = 4 # 图片间隔4个像素
        N_PER_BLOCK = 4 # 这个和batchsize相等，每个block显示4张图片 2*2
        # 循环解包模型，直到获得原始的DDPM模型
        ...
        with torch.no_grad():
            ...
            samples_uncond = model.sample_ddim(N_PER_BLOCK, size).cpu() # 使用DDIM采样器生成N_PER_BLOCK张无条件图像
            ...
            # grid block with only the unconditional samples 将生成的图像排列为网格
            grid = make_grid(samples_uncond, nrow=N_PER_ROW, pad_value=PAD_VALUE, padding=PADDING)
            divider = torch.full((grid.shape[0], PADDING, grid.shape[2]), PAD_VALUE).cpu() # 创建一个垂直分隔条

            if context is not None:
                context = context.float()
                # 2. 正态分布正则化身份向量 N_PER_BLOCK
                # add create synthetic conditional samples block as well and add them to the grid
                syn_context = torch.nn.functional.normalize(torch.randn_like(context)).cuda() # 创建合成条件向量
                samples_syn_cond = model.sample_ddim(N_PER_BLOCK, size, context=syn_context[:N_PER_BLOCK]).cpu() # 使用合成条件向量生成N_PER_BLOCK张图像
                if latent_decoder is not None:
                ...
                # 3. 同一个正态分布正则化身份向量, 测试不同的输入噪声z N_PER_ROW
                ...
                samples_fixed_syn_context = model.sample_ddim(N_PER_ROW, size, context=context_fixed).cpu()
                if latent_decoder is not None:
                ...
                grid = torch.cat([grid, divider, block], dim=1)
                 ...
                 # 4. 不同真实身份向量 N_PER_BLOCK
                # add create authentic conditional samples block as well and add them to the grid
                samples_cond = model.sample_ddim(N_PER_BLOCK, size, context=context[:N_PER_BLOCK]).cpu()
                if latent_decoder is not None:
                ...
                # 同一个真实身份向量，测试不同的输入噪声z N_PER_ROW
                # and fixed context but non-fixed noise conditional generation
                context_fixed = context[0].repeat(N_PER_ROW, 1).cuda()
                samples_fixed_context = model.sample_ddim(N_PER_ROW, size, context=context_fixed).cpu()
                if latent_decoder is not None:
    ...
    def run(self, cfg):
        ...
        # registrate model and optimizer in LightningLite
        ...
        if cfg.latent_diffusion:
            ...
            latent_encoder = VQEncoderInterface(
                ...
                encoder_state_dict_path=os.path.join(cfg.paths.root, cfg.training.checkpoint.VQEncoder)
            
            )
            latent_decoder = VQDecoderInterface(
                first_stage_config_path=os.path.join(cfg.paths.root, "models", "autoencoder", "first_stage_config.yaml"),
                
                decoder_state_dict_path=os.path.join(cfg.paths.root, cfg.training.checkpoint.VQDecoder)
        ...
        while True:
            ...
            for x, context in dataloader:
                t_start = timer()
                count += x.shape[0] # 累计处理的样本数
                if x_visualisation is None:
                    x_visualisation = x.detach().clone() # 保存第一批数据用于可视化，创建独立副本，避免梯度干扰
                    context_visualisation = context.detach().clone() if cfg.model.is_context_conditional else None

                    if cfg.latent_diffusion:
                        with torch.no_grad():
                            sample_size = latent_encoder(x_visualisation[:1]).shape[-3:] # 如果用潜在扩散，编码一张图看潜在空间的尺寸，去最后三个维度(C,H,W)
                    else:
                ...
                # else randomly drop some contexts in case of context-dropout
                    ...
                    dropout_mask = torch.rand(len(x)) < cfg.training.context_dropout # 生成[0,1)之间的随机数,比较得到布尔掩码
                ...
                if context is not None and cfg.model.is_context_conditional and cfg.training.context_permutation > 0:
                    ...
                    permutation = torch.randperm(n_permuted) # 生成随机排列
                    context[:n_permuted] = context[permutation]

                # # evaluation
                # if global_step % cfg.training.steps_between_eval == 0 and global_step != 0:
                ...
                # sampling
                if global_step % cfg.training.steps_between_sampling == 0 and global_step != 0:
                    self.create_and_save_sample_grid(ema_model.averaged_model, size=sample_size,
                                                     x=x_visualisation, context=context_visualisation, latent_decoder=latent_decoder,
                                                     save_path=ensure_path_join(
                                                         os.getcwd(), 'samples', f"sample_{global_step:06d}.png"))
                ...
                loss = diffusion_model(x, context=context.float(), dropout_mask=dropout_mask) # 前向传播计算损失
                self.backward(loss)
                optimizer.step()
