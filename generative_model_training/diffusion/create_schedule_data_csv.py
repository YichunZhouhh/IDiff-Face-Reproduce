...
def create_image_grid_for_schedule(path, schedule_type, **kwargs):
    ...
    for t in list(range(0, T, T // d)) + [T-1]:
        ...
        x_noisy = mean + sd * eps # 此处x_noisy范围[-1, 1]
        x_noisy = (x_noisy * 127.5) + 127.5 # 转换到[0, 255]
...
# LINEAR
alphabars_linear = compute_alphabars('linear')[::d] #每d步取一次
...
all_images_0 = torch.cat(schedule_images, dim=0) #把每个条带按竖直方向拼接到一起
all_images_0 = all_images_0.numpy()

# ======================== Plot that varies parameter beta_min =========================================================

ax = axes[0, 1] #切换第0行一列子图
k = 1
...
all_images0 = clip_and_cast(all_images_0) #有下划线是float numpy数组,无下划线是uint8 numpy数组
