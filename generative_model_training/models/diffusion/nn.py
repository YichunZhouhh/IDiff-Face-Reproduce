...
class SpatialEmbeddingCrossAttentionBlock(torch.nn.Module):
    ...
    结合交叉注意力和自注意力，用于融合图像特征和条件嵌入（如文本、身份特征）
    输入:
  - 图像 x: (B, C, H, W)
  - 嵌入 c: (B, D)

    处理:
  - x: (B, C, H, W) → (B, L, H*W)  # L 是 inner_channels
  - c: (B, D) → (B, D*d) → (B, D, d)  # d 是 n_context_tokens（上下文的token数量）
  ...
  - (B, C, H, W)  # 与输入形状相同
    ...
    def __init__(self, in_channels, context_dim, inner_channels=None, n_context_tokens=None,
        ...
        self.x_proj_out = zero_module(torch.nn.Conv2d(self.inner_channels, in_channels, 1)) # 初始化为0，make the entire block represents as equal map,out=in+0=in,保证训练稳定性，避免破坏预训练模型

        self.self_attention = MultiHeadAttention(in_channels=self.inner_channels, n_heads=n_heads,
                                                 head_channels=head_channels, use_checkpoint=use_checkpoint) # q,k,v都来自都来自图像特征

        d = head_channels if n_context_tokens is None else n_context_tokens # 每个维度扩展成多少个token
        self.c_proj_in = torch.nn.Linear(context_dim, self.context_dim * d) # 把嵌入向量扩展成序列
        ...
        self.cross_attention = MultiHeadAttention(in_channels=self.inner_channels, key_value_channels=context_dim,
                                                  ...
                                                  use_checkpoint=use_checkpoint) # k,v的channel是context_dim，q的channel是inner_channels
