...
class ControlledUnetModel(torch.nn.Module):
    ...
    def __init__(self, only_mid_control: bool = False):
        super().__init__()
        self.only_mid_control = only_mid_control # 是否只在中间块注入控制信号
        self.control_scales = [1.0] * 13 # 每个对应一层的控制强度 1个初始层 + 若干下采样块 + 1个中间块 = 13层
        self.iDiff_unet = None # 预训练的扩散UNet
    ...
    def forward(self, x, t, context = None, hint = None, dropout_mask = None):
        hs = [] #保存跳跃连接的列表
        ### freezed iDiff unet ###
        with torch.no_grad(): #冻结预训练UNet
            # time & first block
            t_s = self.iDiff_unet.time_emb(t) # t_s：(batch,) → (batch, 256)（时间嵌入）
            x_s = self.iDiff_unet.image_proj(x) # x_s：(batch, 3, H, W) → (batch, 64, H, W)（图像投影）
        ...
        ### control unet ###
        control = self.control_unet(x, hint, t, context, dropout_mask) # 原始x,t,c
        # 输出对应层控制信号
        if control is not None:
            x_s += control.pop() #中间块输出+中间块控制
...
class ControlledUnetModel4Sample(torch.nn.Module):
    def __init__(self,
                 eps_model: torch.nn.Module, # 噪声预测模型，这里传入的是 ControlledUnetModel
                 T: int = 1000,
    ...
    def forward(self, x0: torch.Tensor, context: torch.Tensor = None, hint: torch.Tensor = None, dropout_mask: torch.Tensor = None ) -> torch.Tensor:
        # t ~ U(0, T)
        t = torch.randint(0, self.T, (x0.shape[0],)).to(x0.device) # 随机采样时间步
        # eps ~ N(0, 1)
        eps = torch.randn_like(x0) # 采样标准高斯噪声，这是真实噪声
    ...
    def sample(self, n_samples, size, x_T: torch.Tensor = None, context: torch.Tensor = None, hint: torch.Tensor = None, dropout_mask: torch.Tensor = None) -> torch.Tensor:  
        # if initial noise is not provided then sample it （n_samples, 3, 256, 256）
