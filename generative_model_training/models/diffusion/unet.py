...
class SinusoidalTimeEmbedding(torch.nn.Module):
    ...
    def __init__(self, n_channels: int, max_period: int = 10000):
        """
        n_channels: embedding dim of output
        max_period: the max period of sin
        """
        super().__init__()
        ...
        ) # torch.arange(0, half) 生成 [0, 1, 2, ..., half-1],除以 half 归一化到 [0, 1),乘以 -log(max_period) 得到 [0, -log(10000)],exp(...) 得到 [1, 1/10000] 的指数衰减频率
    ...
    def forward(self, t: torch.Tensor):
        ...
        args = t[:, None].float() * frequencies[None] # t->(batch,1) freg->(1,half)

        emb = torch.cat([torch.cos(args), torch.sin(args)], dim=-1) #(batch, half) + (batch, half) → (batch, input_channels)

        emb = self.lin1(emb)
        emb = self.act(emb)
        emb = self.lin2(emb) # final time embedding
...
class ResidualBlock(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, c: torch.Tensor = None):
        h = self.conv1(self.act1(self.norm1(x))) # in_channels->out_channels

        # Addition + GroupNorm (Ho et al.)
        if self.condition_type.lower() == 'addplusgn':
            h += self.time_emb(t)[:, :, None, None] # (b,out_c,h,w)+(b,out_c,1,1)
        ...
        # Adaptive Group Normalization (Dhariwal et al.)
            ...
            if self.is_context_conditional and self.condition_type.lower() == 'adagn':
                emb += self.context_emb(c) # time_emb+context_emb
...
class UpBlock(torch.nn.Module):
    """
    输入通道是 in_channels + out_channels，因为会拼接跳跃连接
    """
    def __init__(self,
                 in_channels: int,
                 out_channels: int,
...
class Upsample(torch.nn.Module):
    ...
    def __init__(self, n_channels):
        super().__init__()
        self.conv = torch.nn.ConvTranspose2d(n_channels, n_channels, (4, 4), (2, 2), (1, 1)) # 分辨率*2
...
class Downsample(torch.nn.Module):
    ...
    def __init__(self, n_channels):
        super().__init__()
        self.conv = torch.nn.Conv2d(n_channels, n_channels, (3, 3), (2, 2), (1, 1)) #分辨率/2
...
class ControlNet(nn.Module):
    ...
    def __init__(
        ...
        # from ControlNet
        # out_channels是实际特征通道数，ch是用于零卷积的参考通道数
        ch = model_channels
        self._feature_size = model_channels #记录所有层的通道数总和
        input_block_chans = [model_channels] #记录每一层的通道数
        ### copy from idiff unet structure ###
    ...
    def forward(self, x, hint, timesteps, context, dropout_mask = None,**kwargs):
        ...
        # 原始hint（如边缘图）的信息密度低，需要提取多层次特征，下采样到与主网络相同的分辨率
        guided_hint = self.input_hint_block(hint, emb, context) # h'=h/8,w'=w/8 三次downsample
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, context: torch.Tensor = None, dropout_mask: torch.Tensor = None):
        ...
        for m in self.up:
            if isinstance(m, Upsample):
                ...
                """
                x: (batch, 128, 32, 32) （上采样路径）
                s: (batch, 128, 32, 32) （跳跃连接）
                torch.cat(): (batch, 256, 32, 32)
                """
                x = m(x, t, c)
        ...
        noise_pred = self.final(self.act(self.norm(x))) # 预测的噪声(b,3,h,w)
