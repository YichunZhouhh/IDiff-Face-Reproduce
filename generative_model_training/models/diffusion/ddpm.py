...
class DenoisingDiffusionProbabilisticModel(torch.nn.Module):
    def __init__(self,
        ...
        self.alphas_next = torch.cat((self.alpha_bars[1:], self.alphas_next), 0)
    ...
    def sample_ddim(self, n_samples, size, x_T: torch.Tensor = None, context: torch.Tensor = None, dropout_mask: torch.Tensor = None, eta = 0, ddim_step = 50) -> torch.Tensor:  
        ...
        return x_t, x_T_array

    def sample_ddim_reverse(self, x_start, n_samples, size, x_T: torch.Tensor = None, context: torch.Tensor = None, dropout_mask: torch.Tensor = None, eta = 0, ddim_step = 20, reverse_steps=None) -> torch.Tensor:  
       
        x_t = x_T if x_T is not None else x_start.cuda()
        skip = self.T // ddim_step
        # 决定反向的终点
        if reverse_steps is not None:
            T_end = reverse_steps * skip  # 只改终点
        else:
            T_end = self.T
        print(f"skip: {skip}, reverse_steps: {reverse_steps}, T_end: {T_end}")
        self.eval()

        x_T_array = [x_start.cpu().detach().numpy()]

        with torch.no_grad():       
            # reverse + reverse = not reverse
            for i in range(0, T_end, skip):
                t = torch.tensor(i).repeat(n_samples).cuda()     
                model_output = self.eps_model(x_t, t, context, dropout_mask)          
                # 1. get previous step value (=t + 1)
                ...
                # 7. compute x_t without "random noise" of formula (12) from https://arxiv.org/pdf/2010.02502.pdf
                x_t = alpha_prod_t_next ** (0.5) * pred_original_sample + pred_sample_direction
    ...
    def sample_ddim_reverse_from_timesteps(self, x_start, n_samples, t_start, t_end, context: torch.Tensor = None, dropout_mask: torch.Tensor = None, ddim_step = 20) -> torch.Tensor:  
        """
        DDIM reverse 从指定时间步开始
        """
        ...
        with torch.no_grad():       
            # reverse + reverse = not reverse
            # 从t_start到t_end
            for i in range(t_start, t_end, skip):
                t = torch.tensor(i).repeat(n_samples).cuda()     
                model_output = self.eps_model(x_t, t, context, dropout_mask)          
                # 1. get previous step value (=t + 1)
                next_timestep = i + skip
