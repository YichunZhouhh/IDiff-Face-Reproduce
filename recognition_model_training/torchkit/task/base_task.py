...
from .helper import ProbabilisticTransform
...
class BaseTask(object):
    ...
    def make_inputs(self):
        ...
        if self.cfg['RAND_CROP']> 0:
            """FRCSyn applies random cropping as data augmentation"""
            rand_crop_transform =transforms.Compose([
                ...
                transforms.RandomResizedCrop(size=(112,112),
                                            ...
                                            ratio=(0.75,4/3)),
                transforms.ToTensor(),
                # RandomGaussianBlur(kernel_range=(3,9),sigma_range=(0.01, 2.0)),
                # transforms.ColorJitter(brightness=0.5,contrast=0.5, saturation=0.5),
                transforms.Normalize(mean=rgb_mean,std=rgb_std)
            ])
            transform = ProbabilisticTransform(rand_crop_transform, transform,
                        max_prob=self.cfg['RAND_CROP'],
                        total_warmup_epochs=self.cfg['CROP_WARMUP_EP'])
