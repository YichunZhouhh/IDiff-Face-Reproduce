...
if __name__ == "__main__":
    ...
    templates, medias = read_template_media_list(os.path.join(args.meta_dir, template_media_fn))
    ...
    # ============================================================
    start = timeit.default_timer()
    p1, p2, label = read_template_pair_list(os.path.join(args.meta_dir, template_pair_fn))
    stop = timeit.default_timer()
    print('Time: %.2f s. ' % (stop - start))
    ...
    print(args.dataset, list(zip(x_labels, tpr_fpr_row)))

    file_path = os.path.join(os.path.dirname(os.path.dirname(feature_path)), f"test/result_ep{str(args.epoch)}.txt")
    if os.path.isfile(file_path):
        with open(file_path, 'a') as file:
            file.write(f"{args.dataset}\t{tpr_fpr_row}\n")
