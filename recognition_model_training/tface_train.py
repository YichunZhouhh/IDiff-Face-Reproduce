...
class TrainTask(BaseTask):
    ...
    def loop_step(self, epoch):
        ...
        if self.cfg['RAND_CROP'] > 0:
            if hasattr(self.train_loader.dataset.sample_parser, 'transform'):
                self.train_loader.dataset.sample_parser.transform.update_epoch(epoch)
            # self.train_loader.dataset.sample_parser.transform.update_epoch(epoch)
        ...
        for step, samples in enumerate(self.train_loader):
            ...
            if self.amp:
                ...
                # ====添加梯度裁剪====
                # 需要先unscale才能进行梯度裁剪
                self.scaler.unscale_(backbone_opt)
                torch.nn.utils.clip_grad_norm_(backbone.parameters(), max_norm=1.0)
                for head_opt, head in zip(head_opts, heads):
                    self.scaler.unscale_(head_opt)
