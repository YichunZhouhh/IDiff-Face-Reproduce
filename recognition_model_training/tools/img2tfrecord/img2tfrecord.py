...
def parse_args():
    ...
    parser.add_argument('--pts_list', default=None, type=str, required=False,
                        help='path to 5p list')
    parser.add_argument('--src_img_dir', default=None, type=str, required=True,
                        help='path to source images, shall match with img_list')
...
def get_img2lmk(pts_file):
    ...
    with open(pts_file, 'r') as f:
        for line in f:
            ...
            lmk = np.array([float(x) for x in line[1:]], dtype=np.float32)  # modified to align with pts_list
            lmk = lmk.reshape((5, 2))
            filename = line[0]
...
def main():
    ...
    if args.pts_list is not None:
        img2lmk = get_img2lmk(args.pts_list)  # omit if images are already cropped
    count = 0
    cur_shard_size = 0
    cur_shard_idx = -1
    ...
    with open(args.img_list, 'r') as f:
        for line in f:
            ...
            img = cv2.imread(os.path.join(args.src_img_dir, img_path))
            if args.pts_list is not None:
                landmark = img2lmk[img_path]
                img = crop_transform(img, landmark, [112, 112])
