...
def unit_vector(vector):
    ...
    return vector / np.linalg.norm(vector)  # v / ||v||


def angle_between(v1, v2):
    v1_u = unit_vector(v1)
    v2_u = unit_vector(v2)
    return np.arccos(np.clip(np.dot(v1_u, v2_u), -1.0, 1.0)) * 180 / 3.1415 # 先点积，再反余弦得到弧度，最后转为角度


angles = []
for i in range(1, 512):
    a, b = np.arange(i) % 2, np.ones(i) / 2 # np.arange(i) % 2 生成0,1交替的向量，np.ones(i)/2生成全为0.5的向量
